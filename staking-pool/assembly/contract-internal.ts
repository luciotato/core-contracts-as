...
export function getStateFromStorage(): StakingContractState {
    if (DEBUG) logging.log("getStateFromStorage")
    // try to read from storage
    if (storage.hasKey("STATE")) {
        if (DEBUG) logging.log("storage.get STATE")
        createdState = storage.get<StakingContractState>("STATE") as StakingContractState
        if (DEBUG) logging.log("after storage.get<StakingContractState>('STATE')")
...
export class StakingContractState {
    ...
    stake_public_key_u8Arr: Uint8Array;
...
//            .stake(u128.from(0), this.stake_public_key_u8Arr)
    ...
    saveClientAccount(clientAccountId: AccountId, accountInfo: ClientAccountState): void {
        if (isEmpty(accountInfo)) {
            if (DEBUG) logging.log("accountInfo.isEmpty => deleting " + clientAccountId)
            //delete
            if (this.persistentAccountsMap.contains(clientAccountId)) {
                this.persistentAccountsMap.delete(clientAccountId)
            }
        } else {
            if (DEBUG) logging.log("storing account info for " + clientAccountId)
            //store
            this.persistentAccountsMap.set(clientAccountId, accountInfo)
        }
        if (DEBUG) logging.log("end saveClientAccount " + clientAccountId)
    }
    ...
    clientDeposit(): u128 {
        ...
        if (DEBUG) logging.log("const clientId = Context.predecessor  - " + clientId)
